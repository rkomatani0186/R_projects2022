...
#player with min launch angle: David Fletcher
min_la_player <- q_data %>%
  filter(launch_angle == min(q_data$launch_angle))

#player with max launch angle: Mike Zunino
...
plot_hr <- ggplot(q_df, aes(launch_angle, home_run_for_BIP, label = hitter, color = exit_velo)) + geom_point() + geom_smooth() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Home Run Percentage for BIP") + scale_colour_gradientn(colours = (rainbow(5)))
...
plot_hr3 <- ggplot(q_df3, aes(launch_angle, home_run_for_BIP, label = hitter, color = exit_velo_bin)) + geom_point() + geom_smooth() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Home Run Percentage for BIP")

plot_hr3
```
```{r}
#Plot slugging percentage for balls in play versus launch angle of qualified batters
plot_slug <- ggplot(q_df, aes(launch_angle, slug_for_BIP , label = hitter, color = exit_velo)) + geom_point() + geom_smooth() +
  geom_label_repel() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Slugging Percentage for BIP") + scale_colour_gradientn(colours = (rainbow(5)))

plot_slug
```
```{r}
#Plot batting average for balls in play versus launch angle of qualified batters
plot_ba <- ggplot(q_df, aes(launch_angle, batting_average_for_BIP , label = hitter)) + geom_point() + geom_smooth() +
...
plot_woba <- ggplot(q_df, aes(launch_angle, woba, label = hitter)) + geom_point() + geom_smooth() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Weighted On Base Average")
...
plot_strikeout <- ggplot(q_df, aes(launch_angle, strikeout_percent, label = hitter)) + geom_point() + geom_smooth() +
  geom_label_repel() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Strikeout Percentage")
...
plot_whiff <- ggplot(q_df, aes(launch_angle, whiff_rate, label = hitter)) + geom_point() + geom_smooth() +
  geom_label_repel() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Swing and Miss Percentage")

plot_whiff
```
```{r}
#Plot foul percentage versus launch angle of qualified batters: all pitches swung at
plot_foul <- ggplot(q_df, aes(launch_angle, foul_percent, label = hitter)) + geom_point() + geom_smooth() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Foul Percentage")
...
plot_hr2 <- ggplot(q_df, aes(exit_velo, home_run_for_BIP, label = hitter)) + geom_point() + geom_smooth()  + xlab("Peak Exit Velocity") + ylab("Home Run Percentage for BIP")
...
# Scaled EV vs LA
```{r}
#Function for plotting Scaled Exit Velocity vs Launch Angle for a batter
...
```{r}
plot_scaled_ev_vs_la("Shohei Ohtani")
...
```{r}
#Function for plotting Scaled Exit Velocity vs Launch Angle for a batter, differentiating events by color
plot_scaled_ev_vs_la_by_events <- function(name) {
  ...
  ggplot(reduced_events_data) + 
    ...
    scale_color_manual(values = c("red", "yellow", "green", "black", "blue"))
}
...
```
```{r}
plot_scaled_ev_vs_la_by_events("Jose Altuve")
...
plot_hit_prob_batter <- function(name) {
  batter <- name_converter(name)
  ggplot(new_data_BIP2 %>% filter (player_name == batter)) + geom_point(aes(launch_angle, prob_Hit, color = launch_speed)) + geom_vline(aes(xintercept = as.numeric(la_for_peak_ev(name)[1])), linetype = "dashed") + annotate(geom = "text", label = as.numeric(la_for_peak_ev(name)[1]), x = as.numeric(la_for_peak_ev(name)[1]), y = 0, vjust = 1) + scale_colour_gradientn(colours = (rainbow(5))) + labs(title = paste(name, "Hit Probability")) + scale_x_continuous(name = "Launch Angle (deg)", limits = c(-80, 80)) + scale_y_continuous(name = "Hit Probability", limits = c(-0.1, 1.1)) 
}

ggarrange(plot_hit_prob_batter("Shohei Ohtani"),plot_hit_prob_batter("Jose Altuve"), plot_hit_prob_batter("David Fletcher"), plot_hit_prob_batter("Mike Zunino"))
```
...
plot_hr_prob_batter <- function(name) {
  batter <- name_converter(name)
  ggplot(new_data_BIP2 %>% filter (player_name == batter)) + geom_point(aes(launch_angle, prob_HR, color = launch_speed)) + geom_vline(aes(xintercept = as.numeric(la_for_peak_ev(name)[1])), linetype = "dashed") + annotate(geom = "text", label = as.numeric(la_for_peak_ev(name)[1]), x = as.numeric(la_for_peak_ev(name)[1]), y = 0, vjust = 1) + scale_colour_gradientn(colours = (rainbow(5))) + labs(title = paste(name, "HR Probability")) + scale_x_continuous(name = "Launch Angle (deg)", limits = c(-80, 80)) + scale_y_continuous(name = "HR Probability", limits = c(-0.1, 1.1)) 
}

ggarrange(plot_hr_prob_batter("Shohei Ohtani"),plot_hr_prob_batter("Jose Altuve"), plot_hr_prob_batter("David Fletcher"), plot_hr_prob_batter("Mike Zunino"))
