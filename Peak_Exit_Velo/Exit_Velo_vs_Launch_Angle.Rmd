...
xwoba_data <- function(name, data) {
  xw_data <- data %>%
    filter(player_name == name)
  return(mean(xw_data$estimated_woba_using_speedangle, na.rm = TRUE))
}

s_d <- qualified_batters %>%
  filter(player_name == "Ohtani, Shohei",
         type == 'X')
mean(s_d$woba_value, na.rm = TRUE)
...
la_ev_batter <- function(data) {
  df <- data.frame(hitter = " ", launch_angle = 0, exit_velo = 0, BIP = 0, single = 0, double = 0, triple = 0, home_run = 0, woba = 0, whiff_rate = 0, foul_percent = 0)
...
```
```{r}
#Plot slugging percentage run per balls in play versus launch angle of qualified batters
...
```{r}
#Plot home run per balls in play versus launch angle of qualified batters
plot_foul <- ggplot(q_df, aes(launch_angle, foul_percent, label = hitter)) + geom_point() + geom_smooth() +
  geom_label_repel() + xlab("Launch Angle that Gives Maximum Exit Velocity") + ylab("Foul Percentage for BIP")
