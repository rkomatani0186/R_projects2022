...
compare_plots <- function(name, aa, lower_la, upper_la) {
  ...
  ggplot(combined_data) +
    ...
    scale_y_continuous(name = "Scaled Exit Velocity (mph)", breaks = seq(0.9, 1.1, by = 0.01)) +
    annotate(geom = "text", label = top_10_data$launch_angle[which.max(predict(quadratic_fit))], x = top_10_data$launch_angle[which.max(predict(quadratic_fit))], y = 1, vjust = 1) + 
