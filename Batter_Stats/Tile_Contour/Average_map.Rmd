...
#Obtain slugging percentage
slg <- function(data, player) {
  data <- data %>%
  filter(batter_name == player,
         play_result %in% c("Single", "Double", "Triple", "Home Run", "Error",
                              "Contact Out", "Swinging Strikeout", "Called Strikeout")) %>%
  ...
  summarize(t_Base = sum(Base),
            AB = n(),
            SLG = t_Base / AB)
  return(summary_data$SLG)
}
```


```{r on base percentage}
#obtain on base percentage
on_base_percent <- function(data, player) {
  data <- data %>%
    filter(batter_name == player,
           play_result %in% c("Single", "Double", "Triple", "Home Run", "Error", "Hit by Pitch",
                              "Contact Out", "Swinging Strikeout", "Called Strikeout") | 
             korbb == "Walk") %>%
    mutate(On_Base = ifelse(play_result %in% c("Single", "Double", "Triple", "Home Run", "Error", "Hit by Pitch") | korbb == "Walk", 1, 0))
  data$On_Base[is.na(data$On_Base)] <- 0
  summary_data <- data %>%
    summarize(t_on_base = sum(On_Base),
              AB = n(),
              OBP = t_on_base / AB)
  return(summary_data$OBP)
...
```{r organize data average}
#creating data for 9_dividers and 4 corners for batting average
...
```{r organize data slugging percentage}
#creating data for 9_dividers and 4 corners for batting average
create_slg_data <- function(player) {
  data_slg <- data.frame(SLG = c(slg(sc_c10, player), slg(sc_c11, player), slg(sc_c12, player),
                               slg(sc_c13, player), slg(sc_c1, player), slg(sc_c2, player), slg(sc_c3, player),
                               slg(sc_c4, player), slg(sc_c5, player), slg(sc_c6, player),
                               slg(sc_c7, player), slg(sc_c8, player), slg(sc_c9, player)
                               ),
                       loc_x = c(- 5 / 12 * plate_width / 12, 5 / 12 * plate_width / 12, - 5 / 12 * plate_width / 12, 5 / 12 * plate_width / 12,
                                 - plate_width / 2 / 12 + plate_width / 6 / 12, 0, plate_width / 2 / 12 - plate_width / 6 / 12,
                                 - plate_width / 2 / 12 + plate_width / 6 / 12, 0, plate_width / 2 / 12 - plate_width / 6 / 12,
...
```{r organize data on base percentage}
#creating data for 9_dividers and 4 corners for on base percentage
create_on_base_percent_data <- function(player) {
  data_on_percent <- data.frame(OBP = c(on_base_percent(sc_c10, player), on_base_percent(sc_c11, player), on_base_percent(sc_c12, player),
                               on_base_percent(sc_c13, player), on_base_percent(sc_c1, player), on_base_percent(sc_c2, player), on_base_percent(sc_c3, player),
                               on_base_percent(sc_c4, player), on_base_percent(sc_c5, player), on_base_percent(sc_c6, player),
                               on_base_percent(sc_c7, player), on_base_percent(sc_c8, player), on_base_percent(sc_c9, player)
                               ),
                       loc_x = c(- 5 / 12 * plate_width / 12, 5 / 12 * plate_width / 12, - 5 / 12 * plate_width / 12, 5 / 12 * plate_width / 12,
                                 - plate_width / 2 / 12 + plate_width / 6 / 12, 0, plate_width / 2 / 12 - plate_width / 6 / 12,
                                 - plate_width / 2 / 12 + plate_width / 6 / 12, 0, plate_width / 2 / 12 - plate_width / 6 / 12,
...
```{r plot}
#plotting 9_dividers and 4 corners for batting average
...
plot_b_avg_data("Branden Comia")

```

```{r plot}
#plotting 9_dividers and 4 corners for slugging percentage
plot_slg_data <- function(player) {
  plot <- ggplot(create_slg_data(player), aes(loc_x, loc_y, width = w, height = h)) +
                    geom_tile(aes(fill = SLG), colour = "grey") + scale_fill_gradient(low = "pink", high = "red") +
    geom_text(data = create_slg_data(player)[5:13,], aes(label = round(SLG, 3))) +
    annotate("text", label = round(slg(sc_c10, player), 3), x = - plate_width / 2 / 12, y = top_edge + (top_edge - bottom_edge) / 8) +
    annotate("text", label = round(slg(sc_c11, player), 3), x = plate_width / 2 / 12, y = top_edge + (top_edge - bottom_edge) / 8) +
    annotate("text", label = round(slg(sc_c12, player), 3), x = - plate_width / 2 / 12, y = bottom_edge - (top_edge - bottom_edge) / 8) +
    annotate("text", label = round(slg(sc_c13, player), 3), x = plate_width / 2 / 12, y = bottom_edge - (top_edge - bottom_edge) / 8)
  add_9_dividers(plot)
}

plot_slg_data("Branden Comia")

```

```{r plot}
#plotting 9_dividers and 4 corners for on base percentage
plot_on_base_percent_data <- function(player) {
  plot <- ggplot(create_on_base_percent_data(player), aes(loc_x, loc_y, width = w, height = h)) +
                    geom_tile(aes(fill = OBP), colour = "grey") + scale_fill_gradient(low = "pink", high = "red") +
    geom_text(data = create_on_base_percent_data(player)[5:13,], aes(label = round(OBP, 3))) +
    annotate("text", label = round(on_base_percent(sc_c10, player), 3), x = - plate_width / 2 / 12, y = top_edge + (top_edge - bottom_edge) / 8) +
    annotate("text", label = round(on_base_percent(sc_c11, player), 3), x = plate_width / 2 / 12, y = top_edge + (top_edge - bottom_edge) / 8) +
    annotate("text", label = round(on_base_percent(sc_c12, player), 3), x = - plate_width / 2 / 12, y = bottom_edge - (top_edge - bottom_edge) / 8) +
    annotate("text", label = round(on_base_percent(sc_c13, player), 3), x = plate_width / 2 / 12, y = bottom_edge - (top_edge - bottom_edge) / 8)
  add_9_dividers(plot)
}

plot_on_base_percent_data("Branden Comia")

```

```{r plots}
#plotting AVG, SLG, OBP
ggarrange(plot_b_avg_data("Branden Comia"), plot_slg_data("Branden Comia"), plot_on_base_percent_data("Branden Comia"))
...
HData <- read.csv("C:/Users/12244/CSV_2021/sc.csv")
```
...
b_avg2 <- function(data, player) {
  data <- data %>%
    filter(type == "X" | events %in% c("strikeout", "strikeout_double_play"),
           player_name == player) %>%
...
**Plotting 9 dividers and 4 corners**
```{r}
