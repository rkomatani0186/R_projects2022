...
```{r}
#Strike out versus launch angle
strike_batters <- data %>%
...
#Obtain data with batter name, launch angle that gives max exit velocity, and strikeouts
...
k_data <- la_k_batter(strike_batters)

ggplot(k_data, aes(launch_angle, strikeouts, label = hitter)) + geom_point() + geom_smooth() + geom_text_repel()
```
...
#Obtain amount of Singles, Doubles, Triples, Home runs, average for balls in play, home run percentage for balls in play
...
```{r}
min_max_data <- q_df %>%
  arrange(-home_run_for_BIP) %>%
  .[1:10, ]

#Home run per balls in play versus launch angle of qualified batters
ggplot(q_df, aes(launch_angle, home_run_for_BIP, label = hitter)) + geom_point() + geom_smooth() +
  geom_label_repel()
